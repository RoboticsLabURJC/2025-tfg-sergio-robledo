...
# ----------------------------
# Loss ponderada por estado
# ----------------------------
def weighted_mse_by_estado(pred, target, estados, w_estado_tensor, w_label=None):
    """
    pred/target: (B,2) -> [steer, throttle]
    estados:     (B,)  -> {1,2,3}
    w_estado_tensor: tensor shape (3,) -> pesos para estados 1..3
    w_label: tensor (2,)
    """
    mse_2 = (pred - target) ** 2  # (B,2)

    # Ponderacion por label, de momento entra [1.0,1.0]
    if w_label is not None:
        mse_2 = mse_2 * w_label.view(1, 2)

    cls_idx = (estados - 1).clamp(0, 2)      # (B,)
    ws = w_estado_tensor[cls_idx]            # (B,)
    mse_2 = mse_2 * ws.view(-1, 1)           # (B,2)

    return mse_2.mean()


def compute_estado_weights_from_dataset(dataset: PilotNetDatasetWithEstado, device):
    estados = np.array(dataset.estados, dtype=np.int64)
    c1 = int((estados == 1).sum())
    c2 = int((estados == 2).sum())
    c3 = int((estados == 3).sum())

    counts = torch.tensor([c1, c2, c3], dtype=torch.float32)
    freq = counts / (counts.sum() + 1e-8)
    w = 1.0 / (freq + 1e-8)
    w = w / w.mean()  # media = 1 , se normaliza
    w = w.to(device)

    weights_dict = {1: float(w[0].item()), 2: float(w[1].item()), 3: float(w[2].item())}
    return w, weights_dict, (c1, c2, c3)
...
def main():
    ...
    # Pesos por estado (SOLO TRAIN)
    W_ESTADO, w_dict, (c1, c2, c3) = compute_estado_weights_from_dataset(train_dataset, device)
    
    # Pesos a mano:
    # W_ESTADO = torch.tensor([2.0, 1.0, 2.0], dtype=torch.float32, device=device)
    # w_dict = {1: 2.0, 2: 1.0, 3: 2.0}



    print(f"Counts estado: 1={c1}, 2={c2}, 3={c3}")
    print(f"Pesos por estado: {w_dict}")

    # Ponderación entre steer/throttle, de momento sin ponderar, ambas a 1.0
    W_LABEL = torch.tensor([1.0, 1.0], dtype=torch.float32, device=device)
    ...
    for epoch in range(last_epoch, args.num_epochs):
        ...
        for i, (images, labels, estados) in enumerate(train_loader):
            ...
            out = model(images)
            loss = weighted_mse_by_estado(out, labels, estados, W_ESTADO, w_label=W_LABEL)
        ...
        # -------- Validation (UNWEIGHTED métricas) --------
        model.eval()
        val_mse = 0.0
        val_mae = 0.0
