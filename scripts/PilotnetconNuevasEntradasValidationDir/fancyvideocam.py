...
image_shape = (66, 200, 4)        # ahora 4 canales: RGB + speed
...
def main():
    ...
    heat_decay = 0.985        # cuanto más cercano a 1, más "persistente"
    heat_brush_radius = 10    # radio del "pincel" en píxeles
    heat_gain = 2.0           # multiplica la intensidad (ajusta)
        ...
        while True:
            ...
            if u_v is not None:
                ...
                if mode == "trail":
                    ...
                    # Decay para que sea "en tiempo real" (solo cuando haya frame)
                    # (también lo puedes hacer cada iteración más abajo)
                    # heatmap *= heat_decay  # opcional aquí

                    # Intensidad proporcional a la velocidad (normaliza como prefieras)
                    # speed_norm ya lo tienes (0..1)
                    intensity = (speed_norm ** 1.5) * heat_gain  # curva para destacar velocidades altas

                    # Pincel: suma en un círculo (rápido y suficiente)
            ...
            if camera_image["data"] is not None:
                ...
                if mode == "trail":
                    ...
                    # decay continuo (aquí mejor que en otro sitio)
                    heatmap *= heat_decay

                    # Suaviza para que parezca "mapa de calor" real (barato)
                    heat_blur = cv2.GaussianBlur(heatmap, (0, 0), sigmaX=7, sigmaY=7)

                    # Normaliza a 0..255 para colorear
                    hmax = float(np.max(heat_blur))
                    if hmax > 1e-6:
                        heat_u8 = np.clip((heat_blur / hmax) * 255.0, 0, 255).astype(np.uint8)
                    else:
                        heat_u8 = np.zeros_like(heat_blur, dtype=np.uint8)

                    heat_color = cv2.applyColorMap(heat_u8, cv2.COLORMAP_JET)  # BGR

                    # Mezcla con el frame cenital
                    alpha = 0.55  # peso del heatmap
