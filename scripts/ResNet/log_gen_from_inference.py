...
log_filename = "/home/sergior/Downloads/pruebas/ResNet/logs/InferenceTrack05CC.log"
MODEL_PATH = "experiments/exp_debug_1769786436/trained_models/pilot_net_model_best_123.pth"
EXP_DIR = "./"
LOGS_DIR = os.path.join(EXP_DIR, "logs")

image_shape = (224, 224, 4)         # RGB + speed
...
def main():
    ...
    try:
        while True:
            ...
            h, w, c = mask.shape
            assert h == 600 and w == 800, f"Mask inesperada: {mask.shape}"

            # Crear banda negra de 200 px
            top_padding = np.zeros((200, w, c), dtype=mask.dtype)

            # Concatenar arriba → (800, 800, 3)
            mask = np.vstack((top_padding, mask))

            # Seguridad
            assert mask.shape == (800, 800, 3), f"Mask final incorrecta: {mask.shape}"

            mask_img = Image.fromarray(mask)                 # PIL image
            x = infer_tf(mask_img).unsqueeze(0)              # (1,3,224,224)
            cv2.imshow("Mask (entrada red)", cv2.cvtColor(mask, cv2.COLOR_RGB2BGR))
            
            speed_plane = torch.full((1,1,224,224), speed_norm, dtype=x.dtype, device=x.device)
