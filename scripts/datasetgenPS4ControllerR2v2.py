...
def save_worker():
    while True:
        item = save_queue.get()
        if item is None:
            break  # Señal para cerrar el hilo
        guardar_dato(*item)
...
while running:
    ...
    if image:
                            ...
                            if len(parts) > 1:
                                ...
                                if len(vals) == 2:
                                    ...
                                    if len(rest) == 2:
                                        ...
                                        local_throttle = max(0.0, min(0.5, (val_r2 / 255.0)*0.5))
        ...
        if time.time() > start_time and control_queue:
            ...
            save_queue.put((timestamp, array, mask_rgb, throttle, steer, brake, speed, heading))
...
sock.close()
pygame.quit()
save_queue.put(None)     # Señal para detener el hilo
save_thread.join()       # Esperar a que termine
