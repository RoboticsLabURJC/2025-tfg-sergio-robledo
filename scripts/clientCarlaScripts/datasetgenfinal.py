...
if not os.path.exists(CSV_PATH):
    with open(CSV_PATH, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["rgb_path", "mask_path", "timestamp", "throttle", "steer", "brake", "speed", "heading","estado"])
...
# Almacenar correctamente el dato obtenido
def guardar_dato(timestamp, rgb_img, mask_class_img, accel, steer, brake, speed, heading):
    ...
    with open(CSV_PATH, mode='a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([rgb_path_rel, mask_path_rel, timestamp, accel, steer, brake, speed, heading, estado])
...
# Esperar 3 segundos desde que hace spawn para empezar a tomar datos
...
while running:
    ...
    if image:
        ...
        if center_x is not None:
            ...
            throttle = 0.8 - Kp_throttle * abs_error
            throttle = np.clip(throttle, 0.2, 0.6)#0.6 antes
        ...
        # Empezar pasasdos esos 3 segundos
