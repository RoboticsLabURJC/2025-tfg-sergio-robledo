...
# Guardar en fichero al cerrar
def guardar_waypoints():
    with open("waypoints.txt", "w") as f:
        f.write("waypoints = [\n")
        for loc in waypoints:
            f.write(f"    carla.Location(x={loc.x:.2f}, y={loc.y:.2f}, z={loc.z:.2f}),\n")
        f.write("]\n")
    print("✅ Waypoints guardados en 'waypoints.txt'")

atexit.register(guardar_waypoints)
...
while running:
    ...
    if current_time - last_save_time >= 3.0:
        ...
        print(f"🧭 Waypoint guardado: x={location.x:.2f}, y={location.y:.2f}, z={location.z:.2f}")
